...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    cd $POOLDIR/blocknotify
    sudo sed -i 's/tu8tu5/'$blckntifypass'/' blocknotify.cpp
    sudo make
    cd $POOLDIR/stratum/iniparser
    sudo make
    cd $POOLDIR/stratum
    if [[ ("$BTC" == "y" || "$BTC" == "Y") ]]; then
    sudo sed -i 's/CFLAGS += -DNO_EXCHANGE/#CFLAGS += -DNO_EXCHANGE/' $POOLDIR/stratum/Makefile
    sudo make
    fi
    sudo make
    cd $POOLDIR
    sudo sed -i 's/AdminRights/'$admin_panel'/' $POOLDIR/web/yaamp/modules/site/SiteController.php
    sudo cp -r $POOLDIR/web /var/
    ...
    cd $POOLDIR
    sudo cp -r $POOLDIR/bin/. /bin/
    sudo cp -r $POOLDIR/blocknotify/blocknotify /usr/bin/
    sudo cp -r $POOLDIR/blocknotify/blocknotify /var/stratum/
...
sudo mv $POOLDIR/ $POOLDIR-install-only-do-not-run-commands-from-this-folder
sudo service nginx restart
sudo service php7.0-fpm reload
